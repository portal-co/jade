import type {
  Tenant, TenantCallableExoticHandler, TenantGenerator, TenantPropertyDescriptor,
} from "../tenants/types.ts";

export type PrimordialFactory<T> = (tenant: Tenant) => TenantGenerator<T>;

/** Read a guest descriptor/object only through Tenant operations. */
export function* readGuestDescriptor(
  tenant: Tenant,
  source: object,
): TenantGenerator<TenantPropertyDescriptor> {
  const out: TenantPropertyDescriptor = {};
  for (const key of ["value", "writable", "get", "set", "enumerable", "configurable"] as const) {
...
export function* guestArrayLike(tenant: Tenant, source: object): TenantGenerator<unknown[]> {
  ...
  const numeric = keys.filter((key): key is string =>
...
export function* descriptorObject(
  tenant: Tenant,
...
export function* defineData(
  ...
  key: PropertyKey,
  value: unknown,
  attributes: Partial<TenantPropertyDescriptor> = {},
): TenantGenerator<void> {
  const ok = yield tenant.yieldTenant(tenant.defineProperty(target, key, {
    value, writable: attributes.writable ?? true, enumerable: attributes.enumerable ?? false,
...
/**
 * A tenant-owned callable with private descriptor storage. Native callable
 * exotics deliberately have every trap populated: the Tenant's missing-trap
 * rule stays fail-closed while a builtin's own properties remain tenant-visible.
 */
export function* makeBuiltin(
  tenant: Tenant,
  name: string,
  apply: (thisArg: unknown, args: readonly unknown[]) => TenantGenerator<unknown>,
  construct?: (newTarget: Function, args: readonly unknown[]) => TenantGenerator<object>,
  ...
  const handler: TenantCallableExoticHandler = {
    ...
    *set(_receiver, key, value) {
      ...
      if (descriptor && descriptor.set) {
        yield tenant.yieldTenant(tenant.invokeTrap(descriptor.set, shell, [value]));
      } else if ((!descriptor || descriptor.writable !== false) && (descriptor || extensible)) {
        descriptors.set(key, { value, writable: true, enumerable: true, configurable: true });
      }
    },
    *has(_receiver, key) { return descriptors.has(key); },
    *delete(_receiver, key) {
      if (descriptors.get(key)?.configurable !== false) descriptors.delete(key);
    },
    *ownKeys() {
      return [...descriptors].filter(([, descriptor]) => descriptor.enumerable).map(([key]) => key);
    },
    *ownPropertyKeys() { return [...descriptors.keys()]; },
    ...
    *apply(_receiver, thisArg, args) { return yield tenant.yieldTenant(apply(thisArg, args)); },
  ...
  yield tenant.yieldTenant(defineData(tenant, shell, "name", name, { writable: false, configurable: true }));
  return shell;
}
