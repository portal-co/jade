...
function* lock(
  ...
  for (const key of yield tenant.yieldTenant(tenant.ownPropertyKeys(object))) {
    ...
    const next = { ...descriptor, configurable: false };
    if (freeze && ("value" in next || next.writable !== undefined)) next.writable = false;
...
/** Create the per-tenant Object graph. Primitive boxing deliberately rejects. */
export function* objectPrimordial(tenant: Tenant): TenantGenerator<ObjectPrimordial> {
  ...
  const ObjectFn = yield tenant.yieldTenant(makeBuiltin(
    tenant,
    "Object",
    function* (_thisArg, args) {
      const value = args[0];
      if (value === null || value === undefined) return yield tenant.yieldTenant(tenant.make(ObjectPrototype));
    ...
    function* (_newTarget, args) {
      const value = args[0];
      if (value === null || value === undefined) return yield tenant.yieldTenant(tenant.make(ObjectPrototype));
      if (typeof value === "object" || typeof value === "function") return value as object;
      throw new TypeError("primitive Object boxing is not available in this Jade realm");
    },
  ));
  const result = { Object: ObjectFn, ObjectPrototype };
  ...
  yield tenant.yieldTenant(installMethod(tenant, ObjectFn, "create", function* (_thisArg, args) {
    const proto = args[0];
    if (proto !== null) assertObject(proto, "Object.create prototype must be object or null");
    return yield tenant.yieldTenant(tenant.make(proto as object | null));
  }));
  yield tenant.yieldTenant(installMethod(tenant, ObjectFn, "keys", function* (_thisArg, args) {
    assertObject(args[0]);
    return yield tenant.yieldTenant(tenant.ownKeys(args[0]));
  }));
  yield tenant.yieldTenant(installMethod(tenant, ObjectFn, "hasOwn", function* (_thisArg, args) {
    assertObject(args[0]);
    return yield tenant.yieldTenant(tenant.has(args[0], args[1] as PropertyKey));
  }));
  yield tenant.yieldTenant(installMethod(tenant, ObjectFn, "assign", function* (_thisArg, args) {
    assertObject(args[0]);
  ...
  yield tenant.yieldTenant(installMethod(tenant, ObjectFn, "defineProperty", function* (_thisArg, args) {
    assertObject(args[0]);
  ...
  yield tenant.yieldTenant(installMethod(tenant, ObjectFn, "defineProperties", function* (_thisArg, args) {
    assertObject(args[0]); assertObject(args[1], "descriptors must be an object");
    yield tenant.yieldTenant(tenant.define(args[0], args[1]));
    return args[0];
  }));
  yield tenant.yieldTenant(installMethod(tenant, ObjectFn, "getOwnPropertyDescriptor", function* (_thisArg, args) {
    assertObject(args[0]);
  ...
  yield tenant.yieldTenant(installMethod(tenant, ObjectFn, "getPrototypeOf", function* (_thisArg, args) {
    assertObject(args[0]); return yield tenant.yieldTenant(tenant.getPrototypeOf(args[0]));
  }));
  yield tenant.yieldTenant(installMethod(tenant, ObjectFn, "setPrototypeOf", function* (_thisArg, args) {
    assertObject(args[0]);
    if (args[1] !== null) assertObject(args[1], "prototype must be object or null");
    if (!(yield tenant.yieldTenant(tenant.setPrototypeOf(args[0], args[1] as object | null)))) throw new TypeError("cannot set prototype");
    return args[0];
  }));
  yield tenant.yieldTenant(installMethod(tenant, ObjectFn, "isExtensible", function* (_thisArg, args) {
    assertObject(args[0]); return yield tenant.yieldTenant(tenant.isExtensible(args[0]));
  }));
  yield tenant.yieldTenant(installMethod(tenant, ObjectFn, "preventExtensions", function* (_thisArg, args) {
    assertObject(args[0]);
    if (!(yield tenant.yieldTenant(tenant.preventExtensions(args[0])))) throw new TypeError("cannot prevent extensions");
    return args[0];
  }));
  yield tenant.yieldTenant(installMethod(tenant, ObjectFn, "seal", function* (_thisArg, args) {
    assertObject(args[0]); return yield tenant.yieldTenant(lock(tenant, args[0], false));
  }));
  yield tenant.yieldTenant(installMethod(tenant, ObjectFn, "freeze", function* (_thisArg, args) {
    assertObject(args[0]); return yield tenant.yieldTenant(lock(tenant, args[0], true));
  }));
  yield tenant.yieldTenant(installMethod(tenant, ObjectPrototype, "hasOwnProperty", function* (thisArg, args) {
