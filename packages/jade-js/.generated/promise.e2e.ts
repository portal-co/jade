import { createNativeHostAsyncCapability, hostTaskFromPromise, hostTaskToPromise, MultiTenant, single_tenant } from "./index.ts";
...
for (const tenant of [
    ...
    const async = createNativeHostAsyncCapability({
    ...
    let rejected = false;
    try {
        drive(tenant, promisePrimordial(tenant, createNativeHostAsyncCapability()));
    } catch (error) {
    ...
    const promise = drive(tenant, tenant.invoke(resolve, {
        kind: "apply",
        thisArg: primordial.Promise,
        args: [
            42
        ]
    }));
    ...
    assert(await hostTaskToPromise(async, primordial.promiseRuntime.toHostTask(promise)) === 42, "explicit guest-to-host HostTask conversion must settle");
    const then = drive(tenant, tenant.get(promise as object, "then")) as Function;
    let observed: unknown;
    const callback = drive(tenant, tenant.makeFunction((value: unknown)=>{
        observed = value;
        return value;
    }));
    drive(tenant, tenant.invoke(then, {
        kind: "apply",
        thisArg: promise,
        args: [
            callback
    ...
    assert(await hostTaskToPromise(async, primordial.promiseRuntime.toHostTask(adopted)) === "host task", "only explicit HostTask adoption crosses into guest Promise");
    const reject = drive(tenant, tenant.get(primordial.Promise, "reject")) as Function;
    drive(tenant, tenant.invoke(reject, {
        kind: "apply",
        thisArg: primordial.Promise,
        args: [
            "unhandled"
