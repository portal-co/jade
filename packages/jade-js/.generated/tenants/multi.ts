import type { Tenant as Tenant_, TenantCallableExoticHandler, TenantExoticHandler, TenantInvocation, TenantPropertyDescriptor } from "./types.ts";
...
export class Tenant implements Tenant_ {
    ...
    *make(proto: object | null = null) {
        ...
        this["__jade$tenant$1f522ae325b3bbe2$owned"].add(out);
        this["__jade$tenant$323cb131ca5b2e50$extensible"].set(out, true);
        this["__jade$tenant$eba40b5600cbc067$shadowForKey"]("__proto__").set(out, {
            value: proto,
            writable: true,
            enumerable: false,
            configurable: false
    ...
    *makeCallableExotic(proto: object | null | undefined, handler: TenantCallableExoticHandler) {
        ...
        shell = function(this: unknown, ...args: unknown[]) {
            if (new.target) {
                return tenant.driveTenant(tenant.#callExotic(shell, {
                    kind: "construct",
                    args,
        ...
        yield this.yieldTenant(this.makeFunction(shell, {
            proto: proto ?? null
        }));
        this.#exotics.set(shell, {
            handler,
            callable: handler
        });
        return shell;
    ...
    *invoke(callee: Function, invocation: TenantInvocation) {
        ...
        return yield this.yieldTenant(this.invokeGuestAware(callee, invocation.kind === "apply" ? invocation.thisArg : undefined, args, invocation));
    }
    *get<R = unknown>(obj: object, key: PropertyKey) {
        const exotic = this["__jade$tenant$cc5a8b1a14d5b650$exotics"].get(obj);
        if (exotic) {
            if (!exotic.handler.get) return this["__jade$tenant$32f46994741b262b$missing"]("get");
            return (yield this.yieldTenant(exotic.handler.get(obj, key))) as R;
        }
        const d = this["__jade$tenant$eba40b5600cbc067$shadowForKey"](key).get(obj);
    ...
    *set<V = unknown>(obj: object, key: PropertyKey, value: V) {
        const exotic = this["__jade$tenant$cc5a8b1a14d5b650$exotics"].get(obj);
        if (exotic) {
            if (!exotic.handler.set) return this["__jade$tenant$32f46994741b262b$missing"]("set");
            return yield this.yieldTenant(exotic.handler.set(obj, key, value));
        }
        const shadow = this["__jade$tenant$eba40b5600cbc067$shadowForKey"](key);
        const existing = shadow.get(obj);
        if (existing && "set" in existing) {
            if (existing.set) yield this.yieldTenant(this.invokeTrap(existing.set as Function, obj, [
                value
            ]));
            return;
        }
        if (existing && existing.writable === false) return;
        if (!existing && this["__jade$tenant$323cb131ca5b2e50$extensible"].get(obj) === false) return;
        shadow.set(obj, {
            value,
            writable: true,
            enumerable: true,
    ...
    *ownKeys(obj: object) {
        const exotic = this["__jade$tenant$cc5a8b1a14d5b650$exotics"].get(obj);
        if (exotic) {
            if (!exotic.handler.ownKeys) return this["__jade$tenant$32f46994741b262b$missing"]("ownKeys");
            return yield this.yieldTenant(exotic.handler.ownKeys(obj));
        }
        const keys: PropertyKey[] = [];
        for (const key of this["__jade$tenant$cc5f2de527b49d89$keys"]){
            if (this["__jade$tenant$eba40b5600cbc067$shadowForKey"](key).get(obj)?.enumerable) keys.push(key);
        }
        return keys;
    }
    ...
    *defineProperty(obj: object, key: PropertyKey, descriptor: TenantPropertyDescriptor) {
        ...
        if (current?.configurable === false && "value" in current && current.writable === false && (descriptor.writable === true || ("value" in descriptor && !Object.is(descriptor.value, current.value)))) return false;
    ...
    *setPrototypeOf(obj: object, prototype: object | null) {
        ...
        if (this.#extensible.get(obj) === false && this.#shadowForKey("__proto__").get(obj)?.value !== prototype) return false;
    ...
    *define(target: object, descriptors: object) {
        const exotic = this["__jade$tenant$cc5a8b1a14d5b650$exotics"].get(target);
        if (exotic) {
            if (!exotic.handler.define) return this["__jade$tenant$32f46994741b262b$missing"]("define");
        ...
        for (const k of keys as PropertyKey[]){
            ...
            if (n) this["__jade$tenant$eba40b5600cbc067$shadowForKey"](k).set(target, n.value as PropertyDescriptor);
        }
    }
    *assign(dst: object, src: object) {
        const exotic = this["__jade$tenant$cc5a8b1a14d5b650$exotics"].get(dst);
        if (exotic) {
            if (!exotic.handler.assign) return this["__jade$tenant$32f46994741b262b$missing"]("assign");
    ...
    get ["__jade$tenant$1f522ae325b3bbe2$owned"]() {
        return this.#owned;
    }
    get ["__jade$tenant$cc5a8b1a14d5b650$exotics"]() {
        return this.#exotics;
    }
    get ["__jade$tenant$323cb131ca5b2e50$extensible"]() {
        return this.#extensible;
    }
    get ["__jade$tenant$cc5f2de527b49d89$keys"]() {
        return this.#keys;
    }
    ["__jade$tenant$32f46994741b262b$missing"](...args) {
        return (this.#missing as any)(...args);
    }
    ["__jade$tenant$eba40b5600cbc067$shadowForKey"](...args) {
        return (this.#shadowForKey as any)(...args);
    }
}
