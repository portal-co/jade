...
export function shadowChained<T extends object, X extends keyof T, U, Args extends unknown[]>(f: <T2 extends {
    [X2 in X]: any;
}>(t: T2, key: X, ...args: Args) => U, shadow: WeakMap<any, any>, { Reflect = {
    ...
    return (val, key, ...args)=>{
        ...
        for(;;){
            ...
            vals = [
                ...vals,
                Reflect.getPrototypeOf(val) as T
            ];
            if (shadow.has(val)) vals = [
                ...vals,
                shadow.get(val) as T
            ];
        }
...
export const shadowReflect = ({ Reflect = globalThis.Reflect }: {
    Reflect?: typeof globalThis.Reflect;
} = {})=>({
        ...new Proxy(Reflect, {
            get (target, p, receiver) {
                if (p === "get" || p === "set" || p === "has" || p === "getOwnPropertyDescriptor") return shadowChained((target as any)[p], shadowMap, {
                    Reflect,
                    freeze: p === "set"
                });
