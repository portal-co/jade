...
for (const tenant of [
    ...
    drive(tenant, tenant.invoke(freeze, {
        kind: "apply",
        thisArg: objects.Object,
        args: [
            value
        ]
    }));
    assert(drive(tenant, tenant.isExtensible(value)) === false, "Object.freeze must prevent extensions");
    assert(drive(tenant, tenant.getOwnPropertyDescriptor(value, "x"))?.writable === false, "Object.freeze must make data descriptors non-writable");
    const functions = drive(tenant, functionPrimordial(tenant));
    let dynamicRejected = false;
    try {
        drive(tenant, tenant.invoke(functions.Function, {
            kind: "apply",
            thisArg: undefined,
            args: []
        }));
    } catch (error) {
        dynamicRejected = error instanceof TypeError;
    }
    ...
    assert((drive(tenant, tenant.invoke(ownKeys, {
        kind: "apply",
        thisArg: reflect.Reflect,
        args: [
            value
        ]
    })) as PropertyKey[]).includes("x"), "Reflect.ownKeys must expose non-enumerable keys through tenant API");
    const proxy = drive(tenant, proxyPrimordial(tenant));
    const handler = drive(tenant, tenant.make(null));
    const proxyValue = drive(tenant, tenant.invoke(proxy.Proxy, {
        kind: "construct",
        newTarget: proxy.Proxy,
        args: [
            value,
            handler
        ]
    ...
    const record = drive(tenant, tenant.invoke(revocable, {
        kind: "apply",
        thisArg: undefined,
        args: [
            value,
    ...
    drive(tenant, tenant.invoke(revoke, {
        kind: "apply",
        thisArg: undefined,
        args: []
    }));
    ...
    try {
        drive(tenant, tenant.get(revocableProxy, "x"));
    } catch (error) {
    ...
    const buffer = drive(tenant, tenant.invoke(buffers.ArrayBuffer, {
        kind: "construct",
        newTarget: buffers.ArrayBuffer,
        args: [
            8
        ]
    })) as object;
    assert(drive(tenant, tenant.get(buffer, "byteLength")) === 8, "ArrayBuffer shell uses explicit native hook capability");
    const typed = drive(tenant, typedArraysPrimordial(tenant, nativeBufferHooks, [
        "Uint8Array"
    ]));
    const Uint8ArrayCtor = typed.constructors.get("Uint8Array")!;
    const array = drive(tenant, tenant.invoke(Uint8ArrayCtor, {
        kind: "construct",
        newTarget: Uint8ArrayCtor,
        args: [
            2
        ]
    })) as object;
    ...
    assert(drive(tenant, tenant.get(realm.globalThis, "ArrayBuffer")) === undefined, "BufferHooks must be explicit: bare realm has no ArrayBuffer");
}
