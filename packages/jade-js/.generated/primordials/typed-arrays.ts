...
export type TypedArrayKind = "Int8Array" | "Uint8Array" | "Uint8ClampedArray" | "Int16Array" | "Uint16Array" | "Int32Array" | "Uint32Array" | "Float32Array" | "Float64Array";
type Codec = {
    bytes: number;
...
const codecs: Record<TypedArrayKind, Codec> = {
    Int8Array: {
        bytes: 1,
        get: (v, i)=>v.getInt8(i),
        set: (v, i, x)=>v.setInt8(i, x)
    },
    Uint8Array: {
        bytes: 1,
        get: (v, i)=>v.getUint8(i),
        set: (v, i, x)=>v.setUint8(i, x)
    },
    Uint8ClampedArray: {
        bytes: 1,
        get: (v, i)=>v.getUint8(i),
        set: (v, i, x)=>v.setUint8(i, Math.max(0, Math.min(255, Math.round(x))))
    },
    Int16Array: {
        bytes: 2,
        get: (v, i)=>v.getInt16(i, true),
        set: (v, i, x)=>v.setInt16(i, x, true)
    },
    Uint16Array: {
        bytes: 2,
        get: (v, i)=>v.getUint16(i, true),
        set: (v, i, x)=>v.setUint16(i, x, true)
    },
    Int32Array: {
        bytes: 4,
        get: (v, i)=>v.getInt32(i, true),
        set: (v, i, x)=>v.setInt32(i, x, true)
    },
    Uint32Array: {
        bytes: 4,
        get: (v, i)=>v.getUint32(i, true),
        set: (v, i, x)=>v.setUint32(i, x, true)
    },
    Float32Array: {
        bytes: 4,
        get: (v, i)=>v.getFloat32(i, true),
        set: (v, i, x)=>v.setFloat32(i, x, true)
    },
    Float64Array: {
        bytes: 8,
        get: (v, i)=>v.getFloat64(i, true),
        set: (v, i, x)=>v.setFloat64(i, x, true)
    }
};
export interface TypedArrayPrimordial {
    constructors: ReadonlyMap<TypedArrayKind, Function>;
}
const caches = new WeakMap<Tenant, {
    identity: object;
    primordial: TypedArrayPrimordial;
}>();
class TypedArrayPrimordialImpl implements TypedArrayPrimordial {
        ...
        const value = yield tenant.yieldTenant(tenant.makeExotic(prototype, {
            *get (receiver, key) {
                ...
                if (key === "set") return yield tenant.yieldTenant(makeBuiltin(tenant, "set", function*(_this, args) {
                    const source = that.#records.get(args[0] as object);
                    if (!source) throw new TypeError("TypedArray.set requires a Jade typed array");
                    const start = toIndex(args[1] ?? 0);
                    if (start + source.length > record.length) throw new RangeError("source is too large");
                    for(let i = 0; i < source.length; i++){
                        ...
                        const target = new Uint8Array(codec.bytes);
                        codec.set(new DataView(target.buffer), 0, number);
            ...
            *set (receiver, key, value) {
                ...
                codec.set(new DataView(bytes.buffer), 0, Number(value));
            ...
            *has (receiver, key) {
                const record = that.#records.get(receiver)!;
                return typeof key === "string" && /^(0|[1-9][0-9]*)$/.test(key) && Number(key) < record.length;
            },
            *delete () {},
            *ownKeys (receiver) {
                const record = that.#records.get(receiver)!;
    ...
    *makeConstructor(tenant: Tenant, kind: TypedArrayKind): TenantGenerator<Function> {
        ...
        const ctor = yield tenant.yieldTenant(makeBuiltin(tenant, kind, function*() {
            throw new TypeError(`Constructor ${kind} requires 'new'`);
        }, function*(_target, args) {
            const first = args[0];
            const bufferRecord = that.#buffers.record(tenant, first);
            if (bufferRecord) {
                const offset = toIndex(args[1] ?? 0);
                if (offset % codec.bytes) throw new RangeError("unaligned byteOffset");
            ...
            const length = toIndex(first ?? 0);
            const handle = yield tenant.yieldTenant(that.#hooks.allocate("array-buffer", length * codec.bytes));
            return yield tenant.yieldTenant(that.create(tenant, kind, yield tenant.yieldTenant(that.#buffers.shell(tenant, "array-buffer", handle, that.#objectPrototype)), 0, length));
        }, this.#objectPrototype));
        yield tenant.yieldTenant(defineData(tenant, ctor, "prototype", prototype, {
            writable: false,
            configurable: false
        }));
        return ctor;
...
export function* typedArraysPrimordial(tenant: Tenant, hooks: BufferHooks, kinds: readonly TypedArrayKind[] = Object.keys(codecs) as TypedArrayKind[]): TenantGenerator<TypedArrayPrimordial> {
