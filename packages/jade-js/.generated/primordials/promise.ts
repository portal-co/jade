import { hostTaskFromPromise, hostTaskPromise, isHostTask, type HostAsyncCapability, type HostTask } from "../async-host.ts";
...
export interface PromisePrimordial {
    Promise: Function;
    PromisePrototype: object;
    promiseRuntime: PromiseRuntime;
}
const cache = new WeakMap<Tenant, {
    capability: HostAsyncCapability;
    result: PromisePrimordial;
}>();
export function* promisePrimordial(tenant: Tenant, async: HostAsyncCapability): TenantGenerator<PromisePrimordial> {
    ...
    const isLocal = (value: unknown): value is GuestPromise<unknown> =>value !== null && typeof value === "object" && records.has(value as object);
    const create = (): object =>{
    ...
    const resolveValue = (shell: object, value: unknown)=>{
        ...
        if (value !== null && (typeof value === "object" || typeof value === "function")) {
            let then: unknown;
            try {
                then = tenant.driveTenant(tenant.get(value as object, "then"), false, false);
            } catch (error) {
                settle(shell, "rejected", error);
                return;
            }
            if (typeof then === "function") {
                ...
                try {
                    ...
                    if (!called) settle(shell, "rejected", error);
                }
                return;
            }
        }
    ...
    const enqueueReaction = (parent: object, reaction: Reaction)=>async.enqueueMicrotask(()=>{
            ...
            try {
                ...
                settle(reaction.child, "rejected", error);
            }
        });
    ...
    const then = yield tenant.yieldTenant(makeBuiltin(tenant, "then", function*(thisArg, args) {
        ...
        if (record.state === "pending") record.reactions.push(reaction);
        else enqueueReaction(parent, reaction);
    ...
    const catcher = yield tenant.yieldTenant(makeBuiltin(tenant, "catch", function*(thisArg, args) {
        return yield tenant.yieldTenant(tenant.invoke(then, {
            kind: "apply",
            thisArg,
            args: [
                undefined,
    ...
    const finally_ = yield tenant.yieldTenant(makeBuiltin(tenant, "finally", function*(thisArg, args) {
        const f = args[0];
        if (typeof f !== "function") return yield tenant.yieldTenant(tenant.invoke(then, {
            kind: "apply",
            thisArg,
            args: [
                undefined,
        ...
        const onFulfilled = yield tenant.yieldTenant(makeBuiltin(tenant, "finallyFulfilled", function*(_unused, values) {
            yield tenant.yieldTenant(tenant.invoke(f, {
                kind: "apply",
                thisArg: undefined,
                args: []
            }));
    ...
    PromiseFn = yield tenant.yieldTenant(makeBuiltin(tenant, "Promise", function*() {
        ...
        const shell = create();
        const resolve = yield tenant.yieldTenant(makeBuiltin(tenant, "resolve", function*(_unused, values) {
            resolveValue(shell, values[0]);
        }));
        const reject = yield tenant.yieldTenant(makeBuiltin(tenant, "reject", function*(_unused, values) {
            settle(shell, "rejected", values[0]);
        }));
        try {
            yield tenant.yieldTenant(tenant.invoke(executor, {
                kind: "apply",
                thisArg: undefined,
                args: [
                    resolve,
    ...
    yield tenant.yieldTenant(defineData(tenant, PromiseFn, "prototype", PromisePrototype, {
        writable: false,
        configurable: false
    }));
    const resolve = yield tenant.yieldTenant(makeBuiltin(tenant, "resolve", function*(_unused, args) {
        const shell = create();
        resolveValue(shell, args[0]);
        return shell;
    }));
    const reject = yield tenant.yieldTenant(makeBuiltin(tenant, "reject", function*(_unused, args) {
        const shell = create();
        settle(shell, "rejected", args[0]);
        return shell;
    }));
    ...
    const runtime: PromiseRuntime = {
        ...
        *resolve (value) {
            const shell = create();
            resolveValue(shell, value);
            return shell as GuestPromise<unknown>;
        },
        *reject (reason) {
            const shell = create();
            settle(shell, "rejected", reason);
            return shell as GuestPromise<never>;
        },
        *fromHostTask <T>(task) {
            const shell = create();
            resolveValue(shell, task);
            return shell as GuestPromise<T>;
        },
        toHostTask <T>(value: GuestPromise<T> | unknown): HostTask<T> {
            const promise = new Promise<unknown>((resolveHost, rejectHost)=>{
                ...
                if (record.state === "pending") record.reactions.push({
                    child: create(),
                    fulfilled: undefined,
                    rejected: undefined
                });
        ...
        awaitGuest (value) {
            return this.toHostTask(value);
        },
        awaitHostTask (task) {
            return hostTaskPromise(async, task);
        }
    };
    const result = {
        Promise: PromiseFn,
        PromisePrototype,
        promiseRuntime: runtime
    };
    cache.set(tenant, {
        capability: async,
        result
