...
// Generated by `wasm-pack build --target nodejs` (gitignored; the Rust orchestrator
// rebuilds it on demand when a run requests `wasm-interp`). Imported statically so the
// WASM module initializes before any `wasm-interp` cell executes.
import * as jadeVmWasm from "./pkg/jade_vm_wasm.js";

export interface Job {
  test: string;
  /** `interp` | `wasm-interp` | `jit-t0` | `jit-t1` | `jit-t2` */
  env: string;
  tenant?: string;
  bytecode?: number[];
  body?: string;
...
function runWasmInterpreter(job: Job, ctx: CellContext): unknown {
  if (!job.bytecode) throw new Error("wasm-interp cell without bytecode");
  return jadeVmWasm.run_virtualized(
    new Uint8Array(job.bytecode),
    {},
    0,
...
export async function execute(job: Job): Promise<CellVerdict> {
  ...
  try {
    const value = job.env === "interp"
      ? runInterpreter(job, ctx)
      : job.env === "wasm-interp"
        ? runWasmInterpreter(job, ctx)
        : runJit(job, ctx);
