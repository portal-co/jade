import type {
  ...
  TenantPropertyDescriptor, TenantProvider,
} from "./types.ts";
  ...
  #bridgeHandler(source: object, sourceOwner: TenantProvider, destination: TenantProvider): TenantExoticHandler {
    ...
    const handler: TenantExoticHandler = {
      ...
      *getOwnPropertyDescriptor(_receiver, key) {
        const inKey = yield router.yieldTenant(marshalIn.call(router, key));
        const descriptor = yield sourceOwner.yieldTenant(
          sourceOwner.getOwnPropertyDescriptor(source, inKey as PropertyKey),
        ) as TenantPropertyDescriptor | undefined;
        if (!descriptor) return undefined;
        const out: TenantPropertyDescriptor = { ...descriptor };
      ...
      *defineProperty(_receiver, key, descriptor) {
        const inKey = yield router.yieldTenant(marshalIn.call(router, key));
        const incoming: TenantPropertyDescriptor = { ...descriptor };
