...
// Fail-closed object traps and tenant-managed exotic state.
{
  const tenant = new MultiTenant();
  let value = 0;
  const exotic = drive(tenant, tenant.makeExotic(null, {
    *get(_receiver, key) { return key === "x" ? value : undefined; },
    *set(_receiver, key, next) { if (key === "x") value = next as number; },
    *has(_receiver, key) { return key === "x"; },
    *delete() { value = -1; },
    *ownKeys() { return ["x"]; },
    *define() {},
    *assign() {},
  ...
  let failed = false;
  try { drive(tenant, tenant.get(missing, "x")); } catch (error) { failed = error instanceof TypeError; }
  assert(failed, "missing exotic trap must fail closed");
}

// apply always gets undefined nt, while construction gets its exact newTarget.
{
  const tenant = new MultiTenant();
  ...
  const fn = drive(tenant, tenant.makeCallableExotic(null, {
    *apply(_receiver, thisArg, args) { applyThis = thisArg; applied = args[0] as number; return applied + 1; },
    *construct(_receiver, newTarget, args) { constructed = newTarget; return { value: args[0] }; },
    *get() {}, *set() {}, *has() { return false; }, *delete() {}, *ownKeys() { return []; }, *define() {}, *assign() {},
  }));
  assert(drive(tenant, tenant.invoke(fn, { kind: "apply", thisArg: "receiver", args: [4] })) === 5, "callable exotic apply result");
  assert(applyThis === "receiver" && applied === 4, "apply receiver and arguments preserved");
  const instance = drive(tenant, tenant.invoke(fn, { kind: "construct", newTarget: fn, args: [8] })) as { value: number };
  assert(instance.value === 8 && constructed === fn, "construct must preserve newTarget");
}

// Independently created secondary objects become bridges under the merged router.
{
  ...
  const merged = new MergedTenant({ primary, providers: [primary, secondary] });
