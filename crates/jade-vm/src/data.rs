    ...
    pub fn parse(buf: &[u8]) -> Option<(Operation, &[u8])> {
        ...
        match op as u16 {
            ...
            1 => {
                let (a, no) = read_u32_le(buf, off)?;
                off = no;
                Some((Operation::Await(crate::Operand::decode(a)), &buf[off..]))
            }
            2 => {
                let (a, no) = read_u32_le(buf, off)?;
                off = no;
                Some((Operation::Yield(crate::Operand::decode(a)), &buf[off..]))
            }
            3 => {
                let (a, no) = read_u32_le(buf, off)?;
                off = no;
                Some((Operation::Yieldstar(crate::Operand::decode(a)), &buf[off..]))
            }
            4 => Some((Operation::Global, &buf[off..])),
            5 => {
                let mut arr = [crate::Operand::decode(0); 4];
                for i in 0..4 {
                    let (x, no) = read_u32_le(buf, off)?;
                    off = no;
                    arr[i] = crate::Operand::decode(x);
                }
                Some((Operation::Fn(arr), &buf[off..]))
            }
            6 => {
                let (a, no) = read_u32_le(buf, off)?;
                off = no;
                Some((Operation::Lit32(crate::Operand::decode(a)), &buf[off..]))
            }
            #[cfg(feature = "alloc")]
            7 => {
                ...
                let (dest, no3) = read_u32_le(buf, off)?;
                off = no3;
                Some((
                    Operation::Arr(items, crate::Operand::decode(dest)),
                    &buf[off..],
                ))
            }
            ...
            8 => {
                ...
                off = no3;
                Some((
                    Operation::Str(items, crate::Operand::decode(dest)),
                    &buf[off..],
                ))
            }
            #[cfg(not(feature = "alloc"))]
    ...
    pub fn emit(&self) -> impl Iterator<Item = u8> {
        ...
        match self {
            ...
            Operation::Arr(items, dest) => {
                ...
                wtr.extend_from_slice(&dest.encode().to_le_bytes());
            }
            #[cfg(not(feature = "alloc"))]
            Operation::Arr => { /* alloc disabled: cannot emit */ }
            #[cfg(feature = "alloc")]
