...
// This crate supplies `WasmPlatform`: the `jade_vm_core::Platform` impl
...
use jade_vm_core::Platform as _; // bring trait methods into scope
...
impl jade_vm_core::Platform for WasmPlatform<'_> {
    type Value = JsValue;
    type Error = JsValue;
    ...
    fn op_global(&mut self, dest: u32) -> Result<(), JsValue> {
        self.cache.set(dest, self.global_this.clone());
        Ok(())
    }
    ...
    fn op_fn(
        ...
        // Flush before capturing parent_state so getter/setter closures see current values.
        self.cache.flush();
        let parent_state = self.cache.state.clone();
        ...
        self.cache.set(dest, spanned);
        Ok(())
    }

    fn op_lit32(&mut self, dest: u32, val: u32) -> Result<(), JsValue> {
        self.cache.set(dest, JsValue::from_f64(val as f64));
        Ok(())
    }

    fn op_arr(&mut self, items: Vec<JsValue>, dest: u32) -> Result<(), JsValue> {
        let arr = Array::new_with_length(items.len() as u32);
        for (i, v) in items.into_iter().enumerate() {
            arr.set(i as u32, v);
        }
        self.cache.set(dest, arr.into());
        Ok(())
    }

    fn op_str(&mut self, cps: Vec<JsValue>, dest: u32) -> Result<(), JsValue> {
        let s: String = cps
            .into_iter()
            .filter_map(|v| char::from_u32(v.as_f64().unwrap_or(0.0) as u32))
            .collect();
        self.cache.set(dest, JsValue::from_str(&s));
        Ok(())
    }

    fn op_litobj(
        &mut self,
        spread: Option<JsValue>,
        ...
        match key {
            Operand::StateRef(idx) => {
                let target = self.cache.get(idx);
                js_define_properties(&target, &obj);
            }
            Operand::Literal(idx) => {
                self.cache.set(idx, obj);
            }
        }
        Ok(())
    }

    fn op_new_target(&mut self, dest: u32) -> Result<(), JsValue> {
        self.cache.set(dest, self.nt.clone());
        Ok(())
    }
    ...
    fn op_call(
        ...
        let result = if let Some((variant_idx, j, closure_slots)) = registry_get(&fn_val) {
            if variant_idx == 0 {
                ...
                Reflect::apply(fn_val.unchecked_ref::<Function>(), &JsValue::UNDEFINED, &call_args)
                    .unwrap_or(JsValue::UNDEFINED)
            }
        } else {
            self.cache.flush_and_invalidate();
